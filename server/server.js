...
app.post('/return', (req, res) => {
    ...
    for (let i = 0; i < dbTableCheckedOutBooks.length; i++){
        if (dbTableCheckedOutBooks[i].isb = req.body.isb){
            bookIndex = i
        }
    }

    if (bookIndex === -1){
        res.status(400).send({message: "This book has not been checked out"})
        return
    }
    let returnedBook = dbTableCheckedOutBooks.splice(bookIndex, 1)

    let returnInfo = {
        patronName: req.body.name,
        patronLibraryId: req.body.lcId,
        bookTitle: returnedBook[0].bookTitle,
        isb: returnedBook[0].isb
    }

    dbTableAvailBooks.push(returnedBook)

    res.status(200).send({
        message: `Book returned! Here's your receipt:`,
        receipt: returnInfo
    })
